...
Eligible articles were full research articles that reported analyses of UKB data. 
We give the reason for exclusion for each article we excluded at the full-text level of screening. 
...
We was to ensure our estimates of reporting completeness included the entire version of record.
...
BW and KD screened them to determine if they met the inclusion criteria.
If insufficient information was available to determine a paper’s eligibility, we contacted the authors and requested the required information, if it was unobtainable we excluded the article.
...
* Studies with inaccessible supplementary material. 
* Studies we did consider to be observational epidemiological studies but could not consider them to have cohort, case-control or cross-sectional designs - for example those using prediction models
...
The data extraction form was hosted on Qualtrics. 
...
We made no major changes so the we retained the responses for articles in the second pilot.
See the PDF version of the data extraction form for full information on our variables and how we measured them at https://osf.io/hngcj/.

The coders independently completed the data extraction form for each article assigned to them.
Almost all variables were categorical so coders assigned the most appropriate category for each variable.
...
Figure 1 presents the reporting completeness for each STROBE item, to ensure a comprehensive understanding of our results, Supplementary Table 2 provides the reporting completeness for each subdivision.
...
* appropriateness of strobe for secondary data analysis - e.g. item 10 excluded completely
* kappa statistics -- limitations of double coding for something so subjective
* deviations from systematic review method & protocol
* reverse engineering of reporting guidelines = inappropriate to use as reporting assessment?
  ** Releasing ‘adherence checklists’ with reporting guidelines would help 
...
create_stab1 <- function(){
  # read in rules to add into table
  stab1 <- read.csv("../data/rules.csv", encoding = "UTF-8", na.strings = c(""), stringsAsFactors = F)
  
  # save ids of all variable cols
  cols_id <- grep("variable", colnames(stab1))
  
  # save variables element so can save for loop products
  x <- c()
  
  # paste all variable cols together and save products
  for(i in 1:nrow(stab1)){
    x <- c(x, paste(stab1[i, cols_id], collapse = ","))
  }
  
  # clean variable names to strobe items and remove all missing items and spaces
  x <- gsub(",NA.*| |starred|^s", "", x) %>%
    gsub(",s", ",", .) %>%
    # add spaces back in so more readable
    gsub(",", ", ", .)
  
  # add in as column
  stab1$STROBEItem <- x
  
  colnames(stab1)[colnames(stab1) == "rule"] <- "Rule"
  colnames(stab1)[colnames(stab1) == "STROBEItem"] <- "STROBE Item" 
  # remove all unnecessary cols
  stab1 <- stab1[, -grep("variable|response", colnames(stab1))]
  
  stab1$Rule <- gsub("^Rule = |^Rule =", "", stab1$Rule)
...
pander(stab1, caption = "Supplementary Table 1. Rules for resolving conflicts", justify = c("left", "center"))
```
...
create_stab2 <- function(){
  ...
  stab2$strobe_item <- gsub("^s|starred", "", stab2$strobe_item)
...
pander(stab2_tab, caption = "Supplementary Table 2. Descriptive statistics for reporting quality of subdivisions of all STROBE items")
```

## Supplementary Table 3

```{r, echo = F}
pander(kappa_sub, caption = "Supplementary Table 3. Kappa statistics for reporting quality of subdivisions of all STROBE items")
```
